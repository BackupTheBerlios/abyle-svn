...
class abyle_firewall:
	def __init__(self, dryrun, iptablesbin, fwconfigpath, rulesfile, ipt_xmlconfig, xmlconfig, echocmd, logfile, verbose):
		...
		abyle_output("", "", "", "#########################################    STARTUP    #########################################","blue", self.logfile, self.verbose)
		abyle_output("", "", "", "","blue", self.logfile, self.verbose)
		...
		if not self.tcpabort == "NO":
			cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.tcpabort_file)
			abyle_output("abyle_firewall", cmdErr.readlines(), cmdOut.readlines(), "ipv4 send TCP-RST on full buffer is activated","default", self.logfile, self.verbose)
                else:
                        cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.tcpabort_file)
                        abyle_output("abyle_firewall", cmdErr.readlines(), cmdOut.readlines(), "ipv4 send TCP-RST on full buffer is deactivated","default", self.logfile, self.verbose)

		abyle_output("","","","IPv4 Reply to ICMP Broadcast:", "blue", self.logfile, self.verbose)
			
		if not self.icmpbcastreply == "NO":
			cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.icmpbcastreply_file)
			abyle_output("abyle_firewall", cmdErr.readlines(), cmdOut.readlines(), "ipv4 reply to ICMP Broadcasts is deactivated","default", self.logfile, self.verbose)
                else:
                        cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.icmpbcastreply_file)
                        abyle_output("abyle_firewall", cmdErr.readlines(), cmdOut.readlines(), "ipv4 reply to ICMP Broadcasts is activated","default", self.logfile, self.verbose)

		abyle_output("","","","IPv4 Dynamic-Address-Hack:", "blue", self.logfile, self.verbose)
			
		if not self.dynaddresshack == "NO":
			cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.dynaddresshack_file)
			abyle_output("abyle_firewall", cmdErr.readlines(), cmdOut.readlines(), "ipv4 dynamic address hack activated","default", self.logfile, self.verbose)
                else:
                        cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.dynaddresshack_file)
                        abyle_output("abyle_firewall", cmdErr.readlines(), cmdOut.readlines(), "ipv4 dynamic address hack deactivated","default", self.logfile, self.verbose)
			
		abyle_output("","","","IPv4 FORWARDING:", "blue", self.logfile, self.verbose)
			
		if not self.ipv4forward == "NO":
			cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.ipv4forwardfile)
			abyle_output("abyle_firewall", cmdErr.readlines(), cmdOut.readlines(), "ipv4 forwarding activated","default", self.logfile, self.verbose)
                        ...
                        cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.ipv4forwardfile)
		...
		abyle_output("","","","SYNCOOKIE:", "blue", self.logfile, self.verbose)
                if not self.syncookie == "NO":
                        cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.syncookiefile)
                        abyle_output("abyle_firewall", cmdErr.readlines(), cmdOut.readlines(), "syncookie activated","default", self.logfile, self.verbose)
                else:
                        cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.syncookiefile)
	...
	def buildUpFinish(self, verbose):
		...
		abyle_output("", "", "", "", "blue", self.logfile, self.verbose)
		abyle_output("", "", "", "#########################################    END    #########################################","blue", self.logfile, self.verbose)
		abyle_output("", "", "", "", "blue", self.logfile, self.verbose)
	...
	def buildUp(self,protectedif,fwconfigpath, verbose):
		...
		if os.path.exists(self.fwconfigpath+'/'+self.protectedif):
			...
			if self.proxyarp == "YES":
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.ipv4conf_path+self.protectedif+'/'+self.proxyarp_file)
                		abyle_output("abyle_firewall_buildUp", cmdErr.readlines(), cmdOut.readlines(), "proxy arp activated for "+self.protectedif+"","default", self.logfile, self.verbose)
			else:
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.ipv4conf_path+self.protectedif+'/'+self.proxyarp_file)
                		abyle_output("abyle_firewall_buildUp", "", "", "proxy arp deactivated for "+self.protectedif+"","default", self.logfile, self.verbose)
			...
			if self.srouting == "YES":
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.ipv4conf_path+self.protectedif+'/'+self.srouting_file)
                		abyle_output("abyle_firewall_buildUp", cmdErr.readlines(), cmdOut.readlines(), "allow source routing activated for "+self.protectedif+"","default", self.logfile, self.verbose)
			else:
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.ipv4conf_path+self.protectedif+'/'+self.srouting_file)
                		abyle_output("abyle_firewall_buildUp", "", "", "allow source routing deactivated for "+self.protectedif+"","default", self.logfile, self.verbose)
			...
			if self.icmprdrs == "YES":
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.ipv4conf_path+self.protectedif+'/'+self.icmpredirects_file)
                		abyle_output("abyle_firewall_buildUp", cmdErr.readlines(), cmdOut.readlines(), "icmp redirects activated for "+self.protectedif+"","default", self.logfile, self.verbose)
			else:
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.ipv4conf_path+self.protectedif+'/'+self.icmpredirects_file)
                		abyle_output("abyle_firewall_buildUp", "", "", "icmp redirects deactivated for "+self.protectedif+"","default", self.logfile, self.verbose)
			...
			if self.sicmprdrs == "YES":
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.ipv4conf_path+self.protectedif+'/'+self.secureicmpredirects_file)
                		abyle_output("abyle_firewall_buildUp", cmdErr.readlines(), cmdOut.readlines(), "secure icmp redirects activated for "+self.protectedif+"","default", self.logfile, self.verbose)
			else:
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.ipv4conf_path+self.protectedif+'/'+self.secureicmpredirects_file)
                		abyle_output("abyle_firewall_buildUp", "", "", "secure icmp redirects deactivated for "+self.protectedif+"","default", self.logfile, self.verbose)
			...
			if self.martians == "YES":
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.ipv4conf_path+self.protectedif+'/'+self.martians_file)
                		abyle_output("abyle_firewall_buildUp", cmdErr.readlines(), cmdOut.readlines(), "martians logging activated for "+self.protectedif+"","default", self.logfile, self.verbose)
			else:
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.ipv4conf_path+self.protectedif+'/'+self.martians_file)
                		abyle_output("abyle_firewall_buildUp", "", "", "martians logging deactivated for "+self.protectedif+"","default", self.logfile, self.verbose)
			...
			if self.bootprelay == "YES":
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.ipv4conf_path+self.protectedif+'/'+self.bootprelay_file)
                		abyle_output("abyle_firewall_buildUp", cmdErr.readlines(), cmdOut.readlines(), "dropping packets from 0/8 activated for "+self.protectedif+"","default", self.logfile, self.verbose)
			else:
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.ipv4conf_path+self.protectedif+'/'+self.bootprelay_file)
                		abyle_output("abyle_firewall_buildUp", "", "", "dropping packets from 0/8 deactivated for "+self.protectedif+"","default", self.logfile, self.verbose)
			...
			if self.antispoofing == "YES":
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.ipv4conf_path+self.protectedif+'/'+self.antispoofing_file)
                		...
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.ipv4conf_path+self.protectedif+'/'+self.antispoofing_file)
