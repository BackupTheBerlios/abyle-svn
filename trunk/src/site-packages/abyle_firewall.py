...
class abyle_firewall:
	def __init__(self, dryrun, iptablesbin, fwconfigpath, rulesfile, syncookie, ipv4forward, ipt_xmlconfig, xmlconfig, antispoofingpath, antispoofingfile, echocmd, logfile, verbose):
		...
		if self.verbose:
			abyle_output("", "", "", "","blue", self.logfile)
			abyle_output("", "", "", "Startup Firewall","blue", self.logfile)
			abyle_output("", "", "", "","blue", self.logfile)
			abyle_output("", "", "", "startup time: "+now,"default", self.logfile)
			abyle_output("", "", "", "","blue", self.logfile)
			
		if self.verbose:
			abyle_output("","","","IPv4 FORWARDING:", "blue", self.logfile)
			
		if not self.ipv4forward == "NO":
			cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.ipv4forward)
			if self.verbose:
				abyle_output("abyle_firewall", cmdErr.readlines(), cmdOut.readlines(), "ipv4 forwarding activated","default", self.logfile)
                else:
                        cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.ipv4forward)
			if self.verbose:
                        	abyle_output("abyle_firewall", cmdErr.readlines(), cmdOut.readlines(), "ipv4 forwarding deactivated","default", self.logfile)

		if self.verbose:
			abyle_output("","","","SYNCOOKIE:", "blue", self.logfile)
                if not self.syncookie == "NO":
                        cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 1 > '+self.syncookie)
			if self.verbose:
                        	abyle_output("abyle_firewall", cmdErr.readlines(), cmdOut.readlines(), "syncookie activated","default", self.logfile)
                else:
                        cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.syncookie)
			if self.verbose:
                        	abyle_output("abyle_firewall", cmdErr.readlines(), cmdOut.readlines(), "syncookie deactivated","default", self.logfile)
			...
			if self.verbose:
				abyle_output("abyle_firewall_buildUpFinish_head", "", "", "default-rule: "+drule,"default", self.logfile)
			if not self.dryrun:
	...
	def buildUpFinish(self, verbose):
		...
		if self.verbose:
			abyle_output("","","","SETTING UP DEFAULT RULES:", "blue", self.logfile)
                for drule in self.defaultrules:
			if self.verbose:
				abyle_output("abyle_firewall_buildUpFinish_foot", "", "", "default-rule: "+drule, "default", self.logfile)
			if not self.dryrun:
                		cmdIn, cmdOut, cmdErr = os.popen3(self.iptablesbin+' '+drule)
		if self.verbose:
			abyle_output("", "", "", "", "blue", self.logfile)
			abyle_output("", "", "", "Startup done - "+now,"blue", self.logfile)
			abyle_output("", "", "", "", "blue", self.logfile)
	...
	def buildUp(self,protectedif,fwconfigpath, verbose):
		...
		self.fwconfigpath = fwconfigpath
		if self.verbose:
			abyle_output("abyle_firewall", "", "", "Building a wall of fire in front of "+self.protectedif, "green", self.logfile)
		else:
			self.output = abyle_output("","","","","default", self.logfile)
		...
		if os.path.exists(self.fwconfigpath+'/'+self.protectedif):
			...
			checkWellformed = self.check_well_formedness(tempFileStr)
        		if checkWellformed != "ok":
                		abyle_output("","","",checkWellformed, "red", self.logfile)
                		sys.exit(1)
        		else:
				if self.verbose:
                			abyle_output("","","",self.fwconfigpath+self.protectedif+self.xmlconfig + " is a well-formed xml", "green", self.logfile)
			...
			if self.verbose:
				abyle_output("","","","ANTI SPOOFING:", "blue", self.logfile)
			else:
				self.output.startup(".")
			...
			if self.antispoofing == "YES":
				...
				if self.verbose:
                			abyle_output("abyle_firewall_buildUp", cmdErr.readlines(), cmdOut.readlines(), "anti spoofing for "+self.protectedif+" activated","default", self.logfile)
			else:
                		cmdIn, cmdOut, cmdErr = os.popen3(self.echocmd+' 0 > '+self.antispoofing_path+self.protectedif+'/'+self.antispoofing_file)
				if self.verbose:
                			abyle_output("abyle_firewall_buildUp", "", "", "anti spoofing for "+self.protectedif+" deactivated","default", self.logfile)
			...
			if self.verbose:
				abyle_output("","","","RULES:", "blue", self.logfile)
			else:
				self.output.startup(".")
			...
			for rule in self.rules:
				...
				if self.verbose:
					abyle_output("abyle_firewall_buildUp_rules", "", "", self.protectedif+" "+rule,"default", self.logfile)
				else:
					self.output.startup(".")
				if not self.dryrun:
			...
			if self.verbose:
				abyle_output("","","","PORTFORWARDING:", "blue", self.logfile)
			else:
				self.output.startup(".")
			...
			if self.portforwarding == "YES":
				...
				for portfwd in self.portforwarding:
					if self.verbose:
						abyle_output("abyle_firewall_buildUp_portfwd", "", "", self.protectedif+" "+portfwd, "default", self.logfile)
					if not self.dryrun:	
                				cmdIn, cmdOut, cmdErr = os.popen3(self.iptablesbin+' '+portfwd)
			else:
				if self.verbose:
					abyle_output("abyle_firewall_buildUp_portfwd", "", "", self.protectedif+" "+"PORTFORWARDING DISABLED", "default", self.logfile)
			
			if self.verbose:
				abyle_output("","","","TRANSPARENT PROXY:", "blue", self.logfile)
			else:
				self.output.startup(".")
			...
			if self.tproxy == "YES":
				...
				for transproxy in self.tproxy:
					if self.verbose:
						abyle_output("abyle_firewall_buildUp_transproxy", "", "", self.protectedif+" "+transproxy, "default", self.logfile)
					if not self.dryrun:
                				cmdIn, cmdOut, cmdErr = os.popen3(self.iptablesbin+' '+transproxy)
			else:
				if self.verbose:
					abyle_output("abyle_firewall_buildUp_tproxy", "", "", self.protectedif+" "+"TRANSPARENT PROXY DISABLED", "default", self.logfile)


			if self.verbose:
				abyle_output("","","","LOGGING:", "blue", self.logfile)
			else:
				self.output.startup(".")
			if self.logging == "YES":
				...
				for log in self.logging:
					if self.verbose:
						abyle_output("abyle_firewall_buildUp_log", "", "", self.protectedif+" "+log, "default", self.logfile)
					if not self.dryrun:
	               				cmdIn, cmdOut, cmdErr = os.popen3(self.iptablesbin+' '+log)
			else:
				if self.verbose:
					abyle_output("abyle_firewall_buildUp_logging", "", "", self.protectedif+" "+"LOGGING DISABLED", "default", self.logfile)

			if self.verbose:
				abyle_output("","","","ALLOW PING:", "blue", self.logfile)
			else:
				self.output.startup(".")
			if self.allowping == "YES":
				...
				for ap in self.allowping:
					if self.verbose:
						abyle_output("abyle_firewall_buildUp_allowping", "", "", self.protectedif+" "+ap, "default", self.logfile)
					if not self.dryrun:
                				cmdIn, cmdOut, cmdErr = os.popen3(self.iptablesbin+' '+ap)
			else:
				if self.verbose:
					abyle_output("abyle_firewall_buildUp_allowping", "", "", self.protectedif+" "+"ALLOWPING DISABLED", "default", self.logfile)

			if self.verbose:
				abyle_output("","","","MASQUERADING:", "blue", self.logfile)
			else:
				self.output.startup(".")
			if self.masquerading == "YES":
				...
				for mg in self.masquerading:
					if self.verbose:
						abyle_output("abyle_firewall_buildUp_masquerading", "", "", self.protectedif+" "+mg, "default", self.logfile)
					if not self.dryrun:
						cmdIn, cmdOut, cmdErr = os.popen3(self.iptablesbin+' '+mg)
			else:
				if self.verbose:
					abyle_output("abyle_firewall_buildUp_masquerading", "", "", self.protectedif+" "+"MASQUERADING DISABLED", "default", self.logfile)
			if not self.verbose:
				con_size = string.join(os.popen("stty size").readlines())
				arr_con_size = string.split(con_size," ")
				size = string.atoi(arr_con_size[1])-13
				size = size+4
				a = string.join(os.popen("echo -ne \033[$(("+str(size)+"))G && echo -ne   [DONE] "))
				self.output.startup(a, "blue")

				#self.output.startup("[DONE]", "blue")
		else:
			abyle_output("abyle_firewall", "", "", "ERROR: No directory found for interface "+self.protectedif+" in "+self.fwconfigpath, "red", self.logfile)
